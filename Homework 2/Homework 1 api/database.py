...
class Database:
    ...
    def __init__(self):
        ...
        result = self.table_exists("singers")
        if result == self.ERROR:
            print("[-] Error connecting to database")
            return

        if not result:
            if self.create_singer_table() == self.ERROR:
                print("[-] Error creating singer table")
                return
            if self.populate_singers_table() == self.ERROR:
                print("[-] Error populating singer table")
                return

        result = self.table_exists("songs")
        if result == self.ERROR:
            print("[-] Error connecting to database")
            return

        if not result:
            if self.create_songs_table() == self.ERROR:
                print("[-] Error creating songs table")
                return
            if self.populate_songs_table() == self.ERROR:
                print("[-] Error populating songs table")
                return
    ...
    def create_singer_table(self):
        try:
            self._cursor.execute(
                'CREATE TABLE singers ('
                'id INTEGER PRIMARY KEY, name TEXT, age INTEGER, country TEXT,'
                'last_visit DATETIME, created_at DATETIME, updated_at DATETIME)')
            self._connection.commit()
        except sqlite3:
            return self.ERROR
    ...
    def create_songs_table(self):
        try:
            self._cursor.execute(
                'CREATE TABLE songs ('
                'id INTEGER PRIMARY KEY, song_name TEXT, singer_id INTEGER,'
    ...
    def populate_downloads_table(self):
        for i in range(random.randint(100, 1000)):
            ...
            file_name = random.choice(["movie", "tvshow"]) + str(random.randint(0, random.randint(100, 1000))) + ".mp4"
    ...
    def populate_singers_table(self):
        for i in range(random.randint(300, 500)):
            first_name_list = ['Emily', 'Liam', 'Sophia', 'Noah', 'Emma', 'Ethan', 'Isabella', 'Mason', 'Ava',
                               'Lucas', 'Mia', 'Jacob', 'Charlotte', 'Daniel', 'Lily']
            last_name_list = ['Smith', 'Johnson', 'Williams', 'Jones', 'Brown', 'Miller', 'Davis', 'Garcia',
                              'Rodriguez',
                              'Wilson', 'Martinez', 'Anderson', 'Taylor', 'Thomas', 'Hernandez']
            country_list = ['United States', 'United Kingdom', 'Canada', 'Australia', 'New Zealand', 'Ireland',
                            'Germany']
            name = random.choice(first_name_list) + " " + random.choice(last_name_list)
            age = random.randint(18, 50)
            country = random.choice(country_list)
            last_visit = datetime.now() - timedelta(days=random.randint(1, 60))
            created_at = datetime.now() - timedelta(days=random.randint(61, 120))
            ...
            try:
                self._cursor.execute(
                    'INSERT INTO singers (name, age, country, last_visit, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
                    (name, age, country, last_visit, created_at, updated_at))
                self._connection.commit()
            except sqlite3:
                return self.ERROR
    ...
    def populate_songs_table(self):
        for i in range(random.randint(300, 500)):
            name = "song" + str(random.randint(0, random.randint(100, 1000)))
            singer_id = random.randint(1, 300)
            created_at = datetime.now() - timedelta(days=random.randint(61, 120))
            updated_at = created_at

            try:
                self._cursor.execute(
                    'INSERT INTO songs (song_name, singer_id, created_at, updated_at) VALUES (?, ?, ?, ?)',
                    (name, singer_id, created_at, updated_at))
                self._connection.commit()
            except sqlite3:
                return self.ERROR
    ...
    def clean_database(self):
        try:
            ...
            self._cursor.execute('DROP TABLE songs')
            self._connection.commit()
    ...
    def get_downloads_join_users(self):
        try:
            self._cursor.execute('SELECT * FROM downloads JOIN users ON downloads.user_id = users.id limit 50')
            return self.build_response()
        except sqlite3:
            return self.ERROR

    def get_songs_join_singers(self):
        try:
            self._cursor.execute('SELECT * FROM songs JOIN singers ON songs.singer_id = singers.id limit 50')
            return self.build_response()
        except sqlite3:
            return self.ERROR
